...
class PCIA(nn.Module):
    ...
    def forward(self, s, q, p_fea, coords):
        ...
        if self.args.SPF is True:
            p_fea = p_fea.transpose(2, 1)                                                  # B N C
            s_mean = s.view(way, shot, dims).mean(1, keepdim=True)
            s = s_mean.expand(way, shot, dims).reshape(way * shot, dims)
            g_fea = torch.cat((s, q), dim=0).view(b, 1, dims)                              # B 1 D

            # 1,get key_idx
            center_idx = cos_similarity_batch(g_fea, p_fea, nk=self.args.SPFknn).squeeze()  # B Kc

            # 2,knn
            c_fea = index_points(p_fea, center_idx.long())       # B Kc D
            local_idx = knn_point(self.args.SPFks, p_fea, c_fea)  # B Kc Kl
            grouped_fea = index_points(p_fea, local_idx)         # B Kc Kl D

            # 3, fuse local feature
            grouped_fea = grouped_fea.view(-1, self.args.SPFks, dims).unsqueeze(2)    # B * Kc Kl 1  D

            g_fea = g_fea.expand(b, self.args.SPFknn, dims)
            g_fea = g_fea.reshape(-1, 1, dims).unsqueeze(2)                          # B*Kc 1  1  D
            g_l_feature = torch.cat((grouped_fea, g_fea), dim=1)
            mean_part_fea = self.Conv11(g_l_feature).view(b, self.args.SPFknn, dims)  # B Kc D
